...
export function DuelInviteLobby({ invite, phase, remainingSeconds, nickname, busy, onShare, onRandom, onCancel, onCreate }: Props) {
  ...
  return <section className="duel-invite-lobby page-enter">
    <span className="duel-invite-ticket">1회용 초대장</span>
    <h1>{title}</h1>
    <p>{detail}</p>
    ...
    <div className="duel-you"><small>방장 이름</small><strong>{invite?.hostName ?? nickname}</strong></div>
    ...
    {phase === 'creating' ? <div className="duel-dots" aria-label="초대방 생성 중"><i /><i /><i /></div>
      : waiting ? <div className="duel-invite-actions"><button className="primary-button" onClick={onShare} disabled={busy}>{busy ? '공유창 여는 중…' : '초대 링크 보내기'} <span>↗</span></button><button className="secondary-button" onClick={onRandom} disabled={busy}>기다리기 싫으면 랜덤 대전</button><button className="text-button" onClick={onCancel}>방 닫고 홈으로</button></div>
