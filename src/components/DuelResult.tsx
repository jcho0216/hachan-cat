...
export function DuelResult({ outcome, profile, nickname, busy, onRematch, onInvite, onHome }: Props) {
  ...
  const hitDecision = match.resultKind === 'hits';
  const headline = draw ? '명중 동점. 점수 그대로.'
    : won ? `${myName} 승리!`
    : hitDecision ? `${opponentName} 판정승`
  ...
  return <section className={`duel-result-screen page-enter ${draw ? 'is-draw' : won ? 'is-win' : 'is-loss'}`}>
    <span className="duel-result-stamp">{draw ? 'DRAW' : hitDecision ? 'HIT DECISION' : won ? 'FIRST HAND' : 'TOO LATE'}</span>
    <h1>{draw ? '명중 수 동점.' : won ? '냥탈 성공.' : '냥탈 실패.'}<br /><em>{headline}</em></h1>
    ...
    <div className="duel-result-card">
      ...
      <blockquote>{draw ? '“이번엔 둘 다 똑같이 귀찮았어.”' : won ? '“사람끼리 싸우더니 결국 날 잡네.”' : opponentReaction ? <>“{opponentReaction.taunt}”<small>— 상대 손 자동번역</small></> : '“둘 다 나보다 느린 건 똑같아.”'}</blockquote>
      <div><span>{hitDecision || draw ? '60초 명중' : '내 기록'}<strong>{hitDecision || draw ? `${match.myHits} : ${match.opponentHits}` : local === null ? won ? '기권승' : '못 잡음' : `${(local / 1000).toFixed(2)}초`}</strong></span><span>승부<strong>{draw ? '점수 변동 없음' : hitDecision ? won ? `${myName} 판정승` : `${opponentName} 판정승` : won ? local === null ? '상대 이탈' : `${myName} 먼저` : `${opponentName} 먼저`}</strong></span></div>
    </div>
    {profile && <p className="duel-streak-note">{match.matchSource === 'invite' ? `친구전 ${profile.friendWins}승 ${profile.friendLosses}패 · 주간 리그 점수는 그대로.` : draw ? '무승부라 승패와 연승은 그대로.' : won ? profile.currentStreak >= 2 ? `🔥 ${profile.currentStreak}연승. 이제 슬슬 이름값 중.` : '첫 연승 불씨를 붙였습니다.' : profile.bestStreak ? `최고 ${profile.bestStreak}연승은 아직 기록에 남아 있음.` : '다음 승부터 연승을 셉니다.'}</p>}
