...
export function DuelReady({ match, nickname, countdown }: Props) {
  ...
  return <section className="duel-ready-screen page-enter">
    ...
    <div className="duel-versus">
      <div><small>나</small><strong>{nickname}</strong></div><b>VS</b><div><small>{match.matchSource === 'invite' ? '초대 수락' : match.opponentKind === 'live' ? '접속 중' : '기록 재생'}</small><strong>{match.opponentName}</strong></div>
    </div>
