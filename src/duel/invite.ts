...
function invitationMessage(hostName: string, outcome: DuelOutcome | null) {
  if (!outcome) return `${hostName}, 냥탈전을 걸었음 😼\n도망갈 시간 2분 줌. 링크 열고 직접 붙자.`;
  if (outcome.match.isDraw) return `${hostName}, 무승부는 못 참겠대 😾\n이번엔 둘 중 한 명은 꼭 고양이 잡기. 2분 안에 들어와.`;
  if (outcome.match.didWin) return `${hostName}, 이기고 바로 시비 거는 중 😼\n말로 말고 같은 고양이로 붙자. 2분 안에 입장.`;
  return `${hostName}, 방금 패배가 억울해서 복수전 엶 😿\n2분 안에 들어와. 이번엔 진짜 동시 시작.`;
}
